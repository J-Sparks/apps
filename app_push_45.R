...
library(sodium) #install.packages("sodium")#library(dbplyr) #install.packages("dbplyr")
                             ...
                             tabItem(tabName = "risk_apr_new_stu",
                                     fluidRow(
                                         box(title="Upload a File", width =  12, solidHeader = T, 
                                             fileInput("new_stu_upload", "Upload a .csv file", buttonLabel = "Browse...", accept = c(".csv",".rds")),
                                             p(span("Click browse to import a .csv file")),
                                             verbatimTextOutput("rawdatadim")),
                                         box(title="Upload Data Pre-view", width = 12, solidHeader = T,
...
server <- function(input, output, session) {
    ### connect DB (postagesql) to get user data
    ### creaate password DB (hashing)
    ...
    #     conn <- dbConnect(
    #         RPostgres::Postgres(),
    #         dbname = "", # generated password DB name
    #         host = "",
    #         port = "5432",
    #         user = "<your_username>", #?
    ...
    #     )
    #     # Get the data
    #     user <- dbGetQuery(conn, glue('SELECT * FROM password WHERE user = \"', input$userInp, '\"'))
    ...
    # })
    ### get User DB
    userdata <- data.frame(user = c('Matthew', 'Gary', 'Kelie'), role = c('Manager', 'User', 'User'), password = c('ABC', 'DEF', 'GHI'))
    ...
    observeEvent(input$butLogin, {  ## login button pressed
        ...
        pw_out <- userdata %>% filter(input$userInp == user & input$pwInp == password)
        if (nrow(pw_out) == 0) {  ## user does not exist
            user$login <- FALSE
            user$header <- 'ERROR - UNKNOWN USER'
        } else {
            pw <- as.character(pw_out$password)[[1]]  ## grab password from DB
             if (pw== input$pwInp) {  ## check that it matches user input
                user$login <- TRUE
                user$name <- input$userInp
                user$role <- userdata[userdata$user == input$userInp, 'role']
    ...
    new_applicants_data <- reactive({
         ...
         ext <- read.csv(input$new_stu_upload$datapath, header = input$header)
        
        # ext <- tools::file_ext(input$new_stu_upload$name)
        # switch(ext,
        #        rds = vroom::vroom(input$new_stu_upload$datapath ),
        #        csv = vroom::vroom(input$new_stu_upload$datapath, delim = ","),
        #        #tsv = vroom::vroom(input$new_stu_upload$datapath, delim = "\t"),
    ...
    new_applicants_cleaned <- reactive({
        stutyp = c("B","E")
        admissiontye = c("UR","UD", "UI")
        variable_risk_factor1 <- c("HS_CEEB","HS_NAME","PROGRAM_CODE","PROGRAM_DESC","HS_OFFER_GPA","HS_FOREIGN_LANG")
        ...
        prog_rate <- hs_prog_apr_help_table %>%   select(program_code, prog_apr_rate)  %>% 
            mutate(prog_apr_rate = ifelse( prog_apr_rate <= 0.2, 0.2, prog_apr_rate)) %>%
            unique()   
        ...
        new_applicants_data() %>% 
            mutate(HS_CEEB = ifelse(is.na(HS_CEEB),"Not Available", HS_CEEB)) %>% 
            filter(STUDTYP %in% stutyp) %>% 
            filter(ADMIT_TYPE %in% admissiontye) %>% 
            #select(UWFID, variable_risk_factor1) %>% 
            ...
            janitor::clean_names() %>% 
            left_join(hs_rate, by="hs_ceeb" ) %>% 
            mutate(HS_New  = ifelse(is.na(hs_apr_rate), "Y","N")) %>% 
            left_join(prog_rate, by="program_code") %>% 
            ...
            mutate(HS_Program_Index  = cut(HS_PROG_APR, breaks=c(0, 0.7499, 0.7999, 0.8499, 1.1 ), 
                                           labels = c("<0.75", "[0.75, 0.80)", "[0.80, 0.85)","0.85+")))  %>% 
            mutate("HS_GPA" = hs_offer_gpa,"Foreign_Lang"  = hs_foreign_lang ) %>% 
            filter(!duplicated(uwfid)) %>% 
            filter(HS_GPA > 0.00)
    ...
    output$new_stu_data <- renderDataTable({
        ...
        my_new_data <- new_applicants_cleaned()  
        
        datatable(my_new_data[1:20,])
        
    })
    
    output$cleandatadim <- renderText({
        dim <- dim(new_applicants_cleaned())
        paste("Number of rows in processed data: ", dim[1])
    })
    ## Data preview
    # output$new_stu_data <- renderDT({
    #     #my_new_data <- new_applicants_data()[1:10,]
    #          #datatable(my_new_data()  )
    #         datatable(new_applicants_cleaned()[1:10,])
    #     })
    
    ### Modeling
    model_pre <- reactive({
        #Gender (Male/Female)
        risk_apr_filtered <-  risk_apr_data_set1 %>% filter(Cohort <= 2020)  %>% 
            mutate(APR= as.factor(APR)) %>% 
            mutate(across(where(is.character), as.factor))
        glm(APR ~ HS_New  + HS_GPA  + HS_Program_Index  + Foreign_Lang , data= risk_apr_filtered, family = "binomial")
    })
    
    #  model_add_drop <- reactive({
    #      
    #      
    #  })
    # model_first_term <- reactive({
    #     
    # })
    ### Prediction data
    ...
    result1 <- reactive({
        ...
        result1 <- cbind("prob_prefac"=prob_pre_factor(), new_applicants_cleaned() )  %>% 
            ...
            relocate(risk_prefac, .after = prob_prefac) %>% mutate(uwfid = factor(uwfid)) %>% janitor::clean_names() %>% 
            mutate(prob_prefac = round(prob_prefac, 4))
    })
    
    output$pred_new_applicants <- DT::renderDataTable({
        predicted <- result1() %>% 
            select(prob_prefac,risk_prefac, uwfid,hs_offer_gpa,foreign_lang, hs_new,hs_apr_rate,prog_apr_rate,hs_program_index)
        
        pred_datatable <-  datatable(predicted,
                                     #extensions = "Buttons",
            ...
            formatRound('prob_prefac', digits = 4) %>% 
            formatStyle('prob_prefac',  
                        # align
    ...
    output$downloadriskdata <- downloadHandler(
        ...
        filename = function(){paste0( "Predict_uploaded_Applicants ",  Sys.Date(), ".csv", sep="")},
        content = function(file){
            write.csv(result1(), file)
        }
    )
    ...
    output$enrollment_bar <- renderPlotly({
        ...
        reten_cc <-  if(input$metricdemo == "None"){
            ...
            cc2 <- left_join(cc, count(cc, cohort,  wt = n, name ="nn")) %>% 
