...
from.entry.to.end.packages <- c("shiny","networkD3","tidyverse","DT","plotly","shinydashboard","shinyBS","shinyWidgets","data.table","purrr")
install.packages(setdiff(from.entry.to.end.packages, rownames(istalled.packages())))
...
ui <- dashboardPage( skin = "green",
                     ...
                     dashboardSidebar( width = 300,
                        ...
                        menuItem( text= "Migration FTIC", startExpanded = T,
                            menuSubItem(text = "Entry/Exit Flow Graph",             tabName = "flowchart", icon = icon("fas fa-stream"))
                            #menuSubItem(text = "Data Table",             tabName = "stophere",     icon = icon("fas fa-table"))
                            # sidebarMenu(width = 300,
                            #                        selectInput(inputId = "sankeycollege_temp", 
                            ...
                            #                                    label   = "HS GPA Range", step = 0.01, dragRange = F,
                            #                                    min= 0.00, max= 5.50, value = c(0.0, 5.04) 
                            #                        ),
                            #                        sliderInput(inputId = "uwfgpay1",
                            #                                    label = "Select UWFGPA Year 1", step = 0.01, dragRange = F,
                         ...
                         tabItems(
                             ...
                             tabItem(tabName= "flowchart", 
                                     fluidRow(
                                                ...
                                                column(3,selectInput("choosedirection", label = "Choose a Direction", choices = c("Forward","Backward"), selected = NULL)),
                                                column(3,selectInput("moveentry","Choose an Entry/Exit", choices = NULL , selected = NULL )),
                                                column(2,actionButton("entrysankey", label = "Start Flow Graph Update", class = "btn-lg btn-success")),
                                     ...
                                     fluidRow(
                                         column(8,  box(width = 12,#title = "Moving In/Out", solidHeader = T, status = "success",
                                          ...
                                          br(), 
                                          numericRangeInput(inputId = "sankeydirectionhsgpa", label = "Choose High School GPA", value = c(0.00,5.04), separator = " to ", step=0.01, width = "50%", min=0.00, max = 5.04 )
                                          )),
...
### Sankey category  and entry or final
    ...
    output$flowtable <- renderDT({
        ...
        FlowTable <- flowdata() %>% 
            select(Cohort,"GPA_HIGHSCHOOL"=HSGPA,GPA,"Entry"=VAR1,  "Year2"=VAR2,"Year3"=VAR3,
            ...
            mutate(GPA = ifelse(GPA == 0, NA, GPA)) %>% 
            dplyr::summarise(  MinHSGPA=min(GPA_HIGHSCHOOL, na.rm=T),
                               ...
                               MeanUWFGPATerm1= round(mean(GPA, na.rm=T),2),
